...
def main() -> None:
    ...
    metrics, vectorizer, model = run_tfidf_cv(texts, labels, model_kind="tfidf-lr", folds=args.folds, seed=args.seed)
    save_json(args.results_dir / "logistic_regression.json", {"summary": summarize(metrics), "folds": [m.__dict__ for m in metrics]})
    if args.save_artifact:
        save_sklearn_artifact(args.results_dir / "artifacts" / "logistic_regression", model, vectorizer, {"model_type": "logistic_regression", "apply_preprocess": True})
